  ...
  await db.execute({
    sql: `INSERT INTO listings (item_key, target, title, price_dkk, url, first_seen, last_seen,
          ...
          ON CONFLICT(item_key) DO UPDATE SET
            ...
            price_dkk = COALESCE(excluded.price_dkk, price_dkk),
    ...
    // pin/undismiss.
    // price_dkk bruger COALESCE (behold eksisterende hvis ny er NULL/udeladt) -
    // en gen-tilføjelse UDEN pris (fx fordi feltet blev efterladt tomt) skal
    // IKKE kunne slette en allerede kendt/forhandlet pris. Fundet 2026-08-06
    // da netop dette skete for en brugers forhandlede 100 kr-pris.
